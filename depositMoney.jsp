...
<%
	...
	try{
		...
		if(amount > 0){
			query = "update USERS set balance=" + (amount + r.getInt("balance")) + " where userId='" + session.getAttribute("logged_in_as") + "';";
			st.executeUpdate(query);
			query = "insert into " + session.getAttribute("logged_in_as") + " values(" + amount + ", 'deposit', 'self');";
			st.executeUpdate(query);
			%> <center>Deposit Successful...</center><%
		}
		else{
...
<body style="background: rgb(241,247,252);">
    ...
    <nav class="navbar navbar-light navbar-expand-md navigation-clean-button" style="background: rgb(163,178,217);">
        <div class="container"><a class="navbar-brand" href="/">SlothBank</a><span>Deposit Money</span><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
    ...
    <% } %>
    <h1 style="text-align: center;font-family: 'Alegreya SC', serif;margin-top: 100px;"><strong>Enter the amount to be deposited :</strong></h1>
    <form style="margin-top: 50px;text-align: left;margin-right: 300px;margin-left: 300px;" action="depositMoney.jsp" method="post"><label class="form-label">Amount</label><input class="form-control" type="number" name="amount" style="width: 250px;"><input class="form-control" type="submit" style="width: 250px;"></form>
