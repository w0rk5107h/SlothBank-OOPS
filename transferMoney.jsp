...
<%
	...
	r.next();
	long balance = r.getInt("balance");
	String type = r.getString("type");
	String accNo1 = r.getString("accNo");
	try{
		String accNo = request.getParameter("accNo");
		long amount = Integer.parseInt(request.getParameter("amount"));
		query = "select * from USERS where accNo='" + accNo + "';";
		r = st.executeQuery(query);
		if(r.next()){
			if(amount > 0){
				if(type.equals("savings")){
					if(balance > amount){
						query = "update USERS set balance=" + (balance-amount) + " where userId='" + session.getAttribute("logged_in_as") + "';" ;
						st.executeUpdate(query);			
						query = "insert into " + session.getAttribute("logged_in_as") + " values(" + amount + ", 'transfer', '" + accNo + "');";
						st.executeUpdate(query);
						
						query = "select * from USERS where accNo='" + accNo + "';";
						ResultSet r1 = st.executeQuery(query);
						r1.next();
						long balance1 = r1.getInt("balance");
						String userId = r1.getString("userId");
						query = "update USERS set balance=" + (amount + balance1) + " where userId='" + userId + "';";
						st.executeUpdate(query);
						query = "insert into " + userId + " values(" + amount + ", 'received', '" + accNo1 + "');";
						st.executeUpdate(query);
			...
			%> <center>Invalid Account number...</center><%
		}
	}
	catch(Exception e){
		e.getMessage();
...
<body style="background: rgb(241,247,252);">
    ...
    <nav class="navbar navbar-light navbar-expand-md navigation-clean-button" style="background: rgb(163,178,217);">
        <div class="container"><a class="navbar-brand" href="/">SlothBank</a><span>Transfer Money</span><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
    ...
    <% } %>
    <h1 style="text-align: center;font-family: 'Alegreya SC', serif;margin-top: 100px;"><strong>money transfer :</strong></h1>
    <form style="margin-top: 50px;text-align: left;margin-right: 300px;margin-left: 300px;" action="transferMoney.jsp" method="post"><label class="form-label">To Account number</label><input class="form-control" type="text" name="accNo" style="width: 350px;"><label class="form-label">Amount</label><input class="form-control" type="number" name="amount" style="width: 250px;"><input class="form-control" type="submit" style="width: 250px;"></form>
